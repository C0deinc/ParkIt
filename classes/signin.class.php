...
class Signin{
        ...
        if ($query->execute()) {
            ...
            if ($accountData && password_verify($this->password, $accountData['password'])) {
                ...
                if ($accountData['first_login'] == 1) {
                    ...
                    $sqlUpdate = "UPDATE account SET first_login = 0 WHERE user_id = :id";
                    $queryUpdate = $this->db->connect()->prepare($sqlUpdate);
                    $queryUpdate->bindParam(':id', $this->user_id);
                    $queryUpdate->execute();
    ...
    function is_first_login(){
        ...
        $sql = "SELECT first_login FROM account WHERE user_id = :id";
        $query = $this->db->connect()->prepare($sql);
